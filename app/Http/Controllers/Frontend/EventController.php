...
class EventController extends BaseController
{
    public function index(Request $request)
    {
        $projects = Projects::select('id','title','project_type_id','country_id','city_id', 'image')
                            ->where('project_category',5) 
                            ->get();
        return view('frontend.events.event-list', compact('projects')); 
    }
    ...
    public function addEvent(Request $request)
    {
       ...
       $project_types = ProjectType::pluck('project_type', 'project_type_id')->toArray();
        ...
        return view('frontend.events.event-add', compact('profile_arr', 'project_types')); 
    }
    ...
    {  
        ...
        $request_data['from_time'] = isset($request->from_time) ? $request->from_time:"";
        $request_data['to_time'] = isset($request->to_time) ? $request->to_time:"";
        $request_data['recurrence'] = isset($request->recurrence) ? $request->recurrence:"";
        $request_data['address'] = isset($request->address) ? $request->address:"";
    ...
    public function eventDetails(Request $request, $id )
    {
       ...
       $project_tags = ProjectTag::where('project_id', $id)->where('project_category',5)->get();
         ...
         if($request->ajax()){
          ...
          if($media_cat_type == 1){
            $this->validate($request,[ 
              ...
              $firstAppData['project_category'] = 5;
    ...
    public function editEvent(Request $request, $id)
    {
        ...
        $project =  $project = Projects::with('project_tags')->where('id', $id)->where('project_category',5)->first();
        ...
        $project_types = ProjectType::pluck('project_type', 'project_type_id')->toArray();
        //return $project;
        return view('frontend.events.event-edit', compact('countries','states', 'cities' , 'profile_arr', 'project_types','project')); 
    }
    ...
    {  
        ...
        $updateData = Projects::where('id',$id)->update($request_data);
            ...
            if(count($project_tag) > 0 ){
                ProjectTag::where('project_id', $id)->where('project_category',5)->delete(); 
                
                foreach($project_tag as $key => $value){  
                    
                    ProjectTag::create(['project_id' => $id, 'project_tag' => $value, 'project_category'=>5]); 
                }
            }
