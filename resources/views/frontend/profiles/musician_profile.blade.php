...
@section('content')
                    ...
                    <div class="row">
                        <div class="col-md-5">
                            ...
                            <h5 class="designation-txt">
                                <?php 
                                    $selected_profession = explode(',', $profile_overview->profession);
                                    $professions = \Config::get('global.profile_2_profession');
                                    if(count($selected_profession) > 1) {
                                        $display_profession = '';
                                        $all_selected_profession = '';
                                        foreach ($selected_profession as $key => $value) {
                                            if($key <= 2) {
                                                $display_profession = (!empty($display_profession) ? $display_profession . '/' : '') . $professions[$value];
                                            }   
                                            $all_selected_profession = (!empty($all_selected_profession) ? $all_selected_profession . ' / ' : '') . $professions[$value];
                                        }
                                        echo '<span class="read_less">' . $display_profession . '</span>';
                                        
                                        if($all_selected_profession) {
                                            echo '<span class="read_less hide">' . $all_selected_profession . '</span>';
                                        }
                                        
                                        if($profile_overview->is_agent == 1) {
                                            echo '&nbsp;<i style="color: #FFC300;" class="fa fa-star" title="Represented By An Agent"></i>';
                                        }
                                    }

                                ?>
                                @if(count($selected_profession) > 3)
                                    <button class='see_more_btn btn btn-primary btn-theme btn-xs'>See More</button>
                                @endif
                            </h5>
    ...
    @include('frontend.profiles.shared.profile_music')
    ...
    {{-- Load profile music modal here --}}
    <div class="modal fade" id="add_music_modal" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true">
        <div class="modal-dialog" role="document">
            <div class="modal-content">
                <div class="modal-header">
                    <h5 class="modal-title" id="updateprofilepicModalLabel">@lang('messages.add') @lang('messages.music')</h5>
                    <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                        <span aria-hidden="true">&times;</span>
                    </button>
                </div>
    ...
    @include('frontend.profiles.shared.musician_scripts')   
